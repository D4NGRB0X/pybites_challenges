...
def get_movies():
    ...
    root = get_tree()
    return [movie.attrib['title'] for movie in root.findall('movie')]

def _get_run_time(run_time):
    return int(run_time.strip(' min'))
...
def get_movie_longest_runtime():
    ...
    root = get_tree()
    runtimes = [(movie.get('title'), _get_run_time(movie.attrib['runtime'])) for movie in root.findall('movie')]
    return max(runtimes, key=itemgetter(1))[0]
