...
# complete :
def check_validity(arg):
    if not isinstance(arg, (int, str, Bloodtype)):
        raise TypeError

    if arg not in blood_type_text.keys() and arg not in [bt.value for bt in Bloodtype]:
        raise ValueError


def get_bt_int(arg):
    output = arg
    if isinstance(arg, str) and blood_type_text.get(arg):
        output = blood_type_text[arg].value

    if isinstance(output, Bloodtype):
        output = arg.value
...
def check_bt(donor, recipient):
    ...
    donor = get_bt_int(donor)
    recipient = get_bt_int(recipient)

    return all(i >= 0 for i in _particular_antigen_comp(donor, recipient))
